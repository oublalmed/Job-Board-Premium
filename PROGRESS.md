...
| **Lot 5** | Messagerie (socle) + quotas de contacts | Lot 1, 4 | 🟨 En cours | 2026-07-29 | Sous-lot 5A ✅ : `ContactQuotaService.consumeOneContact()` derrière `ContactQuotaPort`, décrément atomique (UPDATE conditionnel, pas de check-then-act), prouvé par e2e de concurrence contre Postgres réel. Réutilise l'entité `Subscription` existante (Lot 0/4), aucune migration. Reste 5B : entités Conversation/Message + décrément à l'ouverture du fil. |
...
## Détail du Lot 5 — Messagerie & quotas (🟨 En cours)
...
- [ ] **5B** — entités `Conversation`/`Message`, décrément du quota à l'ouverture du fil (premier message), pas par message
...
- **Décrément atomique** : un seul `UPDATE` conditionnel (`WHERE company_id = :id AND status IN (...) AND (ends_at IS NULL OR ends_at > :now) AND contacts_used < contact_quota`), jamais de lecture-puis-écriture. `result.affected` détermine le succès ; en cas de 0 ligne affectée, une lecture de diagnostic (après coup, ne réintroduit pas la race) distingue abonnement inactif de quota épuisé.
...
- **Aucune migration au Lot 5A** : aucun nouveau champ, schéma déjà présent (synchronize). La première vraie migration de ce repo reste à faire, probablement au Lot 6 (facturation) ou 5B si `Conversation`/`Message` l'exigent.
