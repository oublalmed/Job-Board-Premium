...
- **Avant le premier déploiement en environnement partagé** (staging ou production), trois conditions doivent être remplies :
  1. **Migration baseline** capturant l'état actuel des ~15 tables existantes, générée depuis les entités réelles et **prouvée sur une base vierge** (appliquée de zéro, pas seulement générée) — `CreateMessagingTables` doit s'exécuter après elle dans l'ordre, et redevient alors autonome (elle ne référence plus un schéma fantôme).
     ...
     **Adoption de la baseline sur un environnement préexistant.** Sur une base **vierge**, la baseline s'exécute réellement (`migration:run`) — jamais de fake-apply. Sur une base **déjà peuplée par `synchronize: true`** qu'on fait passer sous gouvernance migrations (dev partagé, staging, prod) : `migration:run` échouera (les tables existent déjà, `42P07`), et la seule façon de l'y adopter est d'insérer directement la ligne de la baseline dans la table `migrations` de **cette base précise** (fake-apply), sans exécuter son `up()`. Cette pratique n'est valide que si l'équivalence du schéma réel de **cette base précise** avec la baseline a été re-prouvée d'abord (`schema:log` vide et/ou `pg_dump --schema-only` sans diff, exécutés sur la base cible elle-même, pas déduits d'une preuve faite ailleurs) — un fake-apply sans cette preuve marquerait comme « appliquée » une baseline potentiellement divergente du schéma réel, et fausserait silencieusement toute migration suivante sur cette base. Cas conforme : `jobboard_dev` (2026-07-30, Lot 6A) — équivalence prouvée d'abord (chantier baseline), fake-apply ensuite, seulement à ce moment-là.
...
**Dette résorbée.** Les trois conditions du trigger 1 sont remplies : la CI empêche désormais que ce chemin se rompe à nouveau sans être détecté, exactement comme demandé.

**Trigger 2 également résolu (Lot 6A, 2026-07-30).** La migration `AddSubscriptionsActiveUniqueIndex` (index unique partiel `ON subscriptions (company_id) WHERE status IN ('trial', 'active')`) s'est appliquée après la baseline sans `42P01`, exactement comme prévu ici. Round-trip `up`/`down`/`up` prouvé par requêtes `psql` fraîches, même exigence que pour `CreateMessagingTables` et `BaselineSchema`. Détail dans `PROGRESS.md` (Décisions techniques Lot 6). Ce document reste la trace de la décision et de son contexte ; il n'est pas révoqué par la résorption des dettes qu'il documentait.
