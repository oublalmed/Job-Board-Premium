...
Restent à faire pour clore entièrement le trigger 1 : `synchronize: false` hors dev (déjà la valeur par défaut du code applicatif — voir Décision — reste à l'imposer en CI) et le job CI migration-only. Le trigger 2 (Lot 6) est désormais **débloqué** : la baseline existe, un futur `AddSubscriptionPartialUniqueIndex` (ou nom équivalent) peut s'appliquer après elle sans `42P01`.
